...
export default function DashboardPage() {
  ...
  // Demo notes if user hasn't created any yet
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#0B0F17] text-slate-800 dark:text-slate-100 flex flex-col font-sans transition-colors duration-200">
      {/* Top Navbar Header */}
      <header className="w-full bg-white dark:bg-[#151D2A] border-b border-slate-200/80 dark:border-slate-800 sticky top-0 z-40">
        <div className="max-w-[1600px] mx-auto px-6 h-16 flex items-center justify-between gap-4">
          {/* Brand Logo */}
          <Link href="/dashboard" className="flex items-center gap-3 shrink-0">
            <div className="w-9 h-9 rounded-xl bg-[#00C48C] flex items-center justify-center text-white font-bold shadow-sm">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="w-9 h-9 rounded-full bg-[#E6F8F2] dark:bg-[#00C48C]/20 text-[#059669] dark:text-[#00E5A3] font-bold text-sm flex items-center justify-center ring-2 ring-[#00C48C]/20 shrink-0">
              SN
            </div>
          </div>
        </div>
      ...
      <div className="max-w-[1600px] w-full mx-auto flex-1 flex flex-col md:flex-row p-6 gap-6">
        ...
        <aside className="w-full md:w-60 bg-white dark:bg-[#151D2A] border border-slate-200/80 dark:border-slate-800/80 rounded-2xl p-4 flex flex-col justify-between shrink-0 shadow-sm transition-colors duration-200">
          <div className="space-y-1">
            <button
              onClick={() => setActiveTab("dashboard")}
              className={`w-full px-4 py-2.5 rounded-xl text-sm font-semibold flex items-center gap-3 transition-colors ${
                activeTab === "dashboard"
                  ? "bg-[#E6F8F2] dark:bg-[#00C48C]/20 text-[#059669] dark:text-[#00E5A3]"
            ...
            <Link
              href="/notes/new"
              className="w-full px-4 py-2.5 rounded-xl text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-slate-800/50 flex items-center gap-3 transition-colors"
            >
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-slate-800 space-y-2">
            <button
              onClick={toggleTheme}
              className="w-full px-4 py-2 rounded-xl text-xs font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-between transition-colors"
            >
              <span>{theme === "dark" ? "Dark Theme" : "White / Light Theme"}</span>
              {theme === "dark" ? <Moon className="w-3.5 h-3.5 text-indigo-400" /> : <Sun className="w-3.5 h-3.5 text-amber-500" />}
            </button>

            <button
              onClick={handleLogout}
              className="w-full px-4 py-2.5 rounded-xl text-sm font-medium text-slate-500 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/30 flex items-center gap-3 transition-colors"
            >
        ...
        <main className="flex-1 space-y-6 min-w-0">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white bg-white dark:bg-[#151D2A] border border-slate-200/80 dark:border-slate-800 rounded-xl shadow-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-all">
                <SlidersHorizontal className="w-4 h-4" />
              </button>
              <div className="flex items-center bg-white dark:bg-[#151D2A] border border-slate-200/80 dark:border-slate-800 rounded-xl p-1 shadow-sm">
                <button className="p-1.5 bg-[#E6F8F2] dark:bg-[#00C48C]/20 text-[#059669] dark:text-[#00E5A3] rounded-lg">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
            {filteredNotes.map((note) => {
              ...
              return (
                <div
                  key={note.id}
                  className="bg-white dark:bg-[#151D2A] border border-slate-200/80 dark:border-slate-800/80 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all flex flex-col justify-between space-y-4"
                >
                  <div>
                    {/* Top Badges & Options */}
                    <div className="flex items-center justify-between gap-2 mb-3">
                      <div className="flex items-center gap-2 flex-wrap">
                        {share?.isOneTime && (
                          <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 dark:bg-emerald-950/40 text-[#059669] dark:text-[#00E5A3] border border-emerald-100 dark:border-emerald-800/50">
                            One-Time
                          </span>
                        )}
                    ...
                    <h2 className="text-base font-bold text-slate-900 dark:text-white mb-1.5 truncate">
                      {note.title}
                    </h2>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {isActive && share?.token ? (
                        <button
                          ...
                          className="px-3 py-1.5 bg-[#E6F8F2] dark:bg-[#00C48C]/20 hover:bg-[#d4f3e7] dark:hover:bg-[#00C48C]/30 text-[#059669] dark:text-[#00E5A3] font-semibold text-xs rounded-xl flex items-center gap-1.5 transition-colors"
                        >
                          {copiedToken === share.token ? (
                            <>
                              <Check className="w-3.5 h-3.5 text-[#059669] dark:text-[#00E5A3]" />
                              <span>Copied</span>
                            </>
                          ) : (
                            <>
                        ...
                        <button disabled className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 font-semibold text-xs rounded-xl flex items-center gap-1.5 cursor-not-allowed">
                          <span>Link Dead</span>
                        </button>
                      ) : (
                        <button className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-semibold text-xs rounded-xl flex items-center gap-1.5 transition-colors">
                          <span>Reshare</span>
                        </button>
                      )}
                    </div>
        ...
        <aside className="w-full md:w-80 space-y-6 shrink-0">
          {/* Quick Stats Card */}
          <div className="bg-white dark:bg-[#151D2A] border border-slate-200/80 dark:border-slate-800/80 rounded-2xl p-5 shadow-sm space-y-5 transition-colors duration-200">
            ...
            <div className="grid grid-cols-2 gap-3 text-center">
              <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-xl p-3">
                ...
                <div className="text-2xl font-bold text-[#059669] dark:text-[#00E5A3] mt-0.5">{totalNotes}</div>
              </div>
              <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-xl p-3">
                <div className="text-[11px] font-medium text-slate-400">Active Shares</div>
                <div className="text-2xl font-bold text-[#059669] dark:text-[#00E5A3] mt-0.5">{activeShares}</div>
              </div>
              <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-xl p-3">
                <div className="text-[11px] font-medium text-slate-400">Expired</div>
                <div className="text-2xl font-bold text-amber-600 dark:text-amber-400 mt-0.5">{expiredShares}</div>
              </div>
              <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-xl p-3">
                <div className="text-[11px] font-medium text-slate-400">Revoked</div>
                <div className="text-2xl font-bold text-rose-600 dark:text-rose-400 mt-0.5">{revokedShares}</div>
              </div>
            </div>

            <div className="pt-3 border-t border-slate-100 dark:border-slate-800">
              <div className="flex items-center justify-between text-xs font-semibold mb-2">
                <span className="text-slate-600 dark:text-slate-400">Total Link Views</span>
                <span className="text-slate-900 dark:text-white text-sm font-bold">1.2k</span>
              </div>
          ...
          <div className="bg-white dark:bg-[#151D2A] border border-slate-200/80 dark:border-slate-800/80 rounded-2xl p-5 shadow-sm space-y-4 transition-colors duration-200">
            <h3 className="text-sm font-bold text-slate-900 dark:text-white tracking-tight">
              Activity
            </h3>
            ...
            <div className="space-y-4 text-xs">
              <div className="flex items-start gap-3">
                <div className="w-7 h-7 rounded-full bg-[#E6F8F2] dark:bg-[#00C48C]/20 text-[#059669] dark:text-[#00E5A3] flex items-center justify-center shrink-0 mt-0.5">
              ...
              <div className="flex items-start gap-3">
                <div className="w-7 h-7 rounded-full bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 flex items-center justify-center shrink-0 mt-0.5">
              ...
              <div className="flex items-start gap-3">
                <div className="w-7 h-7 rounded-full bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 flex items-center justify-center shrink-0 mt-0.5">
